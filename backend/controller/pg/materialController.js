...
export const createMaterial = asyncHandler(async (req, res, next) => {
  const { name, initial_number } = req.body
  const client = await getConnection()
  try {
    const query = `
...
export const addMaterials = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      RETURNING *;
    `
    const updateResult = await client.query(updateQuery, [added_number, id])
...
export const subtractMaterials = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    const updateQuery = `
      ...
      RETURNING *;
    `
    const updateResult = await client.query(updateQuery, [damaged_number, id])
